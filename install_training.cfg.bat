...
SET projectname=csgo-training.cfg
...
ECHO %csgo%
ECHO.
IF EXIST "%csgo%/csgo/cfg/training.cfg" ( ECHO [1mUpdating files...[0m ) ELSE ( [1mECHO Installing...[0m )
ECHO.

CD /d %csgo%
ECHO [90m===============================================================================
curl -L -o xioner19.bat.tar.gz https://codeload.github.com/xioner19/%projectname%/tar.gz/master

:DOWNLOADING
TASKLIST /fi "IMAGENAME eq curl.exe" >NUL
IF ERRORLEVEL 1 TIMEOUT /t 1 & GOTO DOWNLOADING

tar -xzf xioner19.bat.tar.gz --strip=1 %projectname%-master/csgo && DEL xioner19.bat.tar.gz
ECHO ===============================================================================[0m
ECHO.
ECHO [92mSuccess![0m
ECHO.
